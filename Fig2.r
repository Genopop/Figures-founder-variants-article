...
SLSJ_res_df <- data.frame(upper=NA,mean=c(SLSJ_res,0),lower=NA,variants_num=c(0:9),group='SLSJ')
barplot_res_df <- rbind(barplot_res_df,SLSJ_res_df)
RQ_res <- as.numeric(hist_like_rq_df)
RQ_res <- c(21472-sum(RQ_res),RQ_res)
barplot_res_df$value <- c(RQ_res,c(SLSJ_res,0))
...
for (i in c(1:1000)){
	...
	resampling_vec <- c(resampling_vec,1153-length(unique(num_var_present)))
}
CI(resampling_vec)
...
counts <- matrix(c(207, 1153-207, 424, 1153-424), nrow = 2, byrow = TRUE)
